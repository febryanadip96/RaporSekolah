...
<div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="{{url('guru/matapelajaran/portofolio/'.$mapelBuka->id)}}">Portofolio (KD)</a></li>
      ...
      <div class="col-xs-12">
          <div class="box box-success">
            <div class="box-header">
              <h3 class="box-title">Portofolio</h3><br>
              <small>Deskripsi: {{$kd->deskripsi}}</small>
            </div>
            <!-- /.box-header -->
            <form role="form" class="form-update" action="{{url('guru/matapelajaran/portofolio/'.$mapelBuka->id.'/kd/'.$kd->id)}}" method="post">
                {{ csrf_field() }}
                <div class="box-body no-padding">
                  <table class="table table-striped">
                    ...
                    <tr>
                      <td>{{$portofolio->nilaiRapor->semesterSiswa->siswa->nis}}</td>
                      <td>{{$portofolio->nilaiRapor->semesterSiswa->siswa->user->name}}</td>
                      <td><input class="nilai" type="number" name="nilai[{{$portofolio->id}}]" min="0" max="100" value="{{$portofolio->nilai}}"></td>
                    </tr>
                    @endforeach
                  </table>
                </div>
